...
IF NOT EXISTS(select * from sysobjects where type = 'U' and name = 'T_Estado_Clientes')
	BEGIN
		create table T_Estado_Clientes
		(
			Id_Estado_Clientes			 int primary key,
			Descripcion_Estado_Clientes  nvarchar(100) not null
		)
	END
ELSE
	BEGIN
		print 'Tabla T_Estado_Clientes ya existe'
	END
go

IF NOT EXISTS(select * from sysobjects where type = 'U' and name = 'T_Roles')
	BEGIN
